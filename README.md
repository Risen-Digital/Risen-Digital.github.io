...
- `see/augusta-downtown-alliance/index.html` — Augusta Downtown Alliance
  pitch page. As of 2026-09-20 (RisenDigitalSite PR #26,
  https://github.com/Risen-Digital/RisenDigitalSite/pull/26) this is no
  longer hand-typed: it is generated by
  `php artisan pitch:export-static augusta-downtown-alliance` from the real
  `PitchPage` row's own stored fields (`hero_data`, `review_section`,
  `information_section`, `execute_section`, `pricing_options`, `insights`) —
  the RisenDigitalSite Postgres row (project #241) is the source of truth;
  this file is a build output of that row, republished here only because
  RisenDigitalSite itself has no production host yet (commitment #6935,
  hosting deferred 2026-09-13).
- `see/augusta-downtown-alliance-rebuild/index.html` — the sample homepage
  rebuild, now its own separately fetchable page (previously combined with
  the pitch page in an iframe). Generated by the same
  `pitch:export-static` command from `settings.rebuild_html` on the same
  `PitchPage` row. Once RisenDigitalSite has production hosting, this maps
  to the native route `/see/augusta-downtown-alliance/rebuild`
  (`PitchPageController@rebuild`).
